...
function AdminAvatar({ admin }) {
  ...
  const initials = name.slice(0, 2).toUpperCase();
  return (
    <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-md shadow-violet-200">
      {initials}
    </div>
  );
}
...
function Sidebar({ collapsed, onToggle }) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="flex items-center h-[60px] px-4 border-b border-slate-100 flex-shrink-0">
        <div className="flex items-center gap-3 overflow-hidden">
        ...
        <button
          onClick={onToggle}
          className={`ml-auto w-6 h-6 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors flex-shrink-0 ${collapsed ? "mx-auto" : ""}`}
          title={collapsed ? "Expand" : "Collapse"}
        >
          <svg className={`w-3.5 h-3.5 transition-transform duration-300 ${collapsed ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round">
            <path d="M15 18l-6-6 6-6" />
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Icon */}
              <span className={`w-5 h-5 flex-shrink-0 transition-colors ${isActive ? "text-violet-600" : "text-slate-400 group-hover:text-slate-600"}`}>
                {item.icon}
              </span>
      ...
      {/* Bottom */}
      <div className="flex-shrink-0 border-t border-slate-100 p-2.5">
        {/* Admin info */}
        <div className={`flex items-center gap-2.5 px-2 py-2 rounded-xl mb-1 overflow-hidden ${collapsed ? "justify-center" : ""}`}>
          <AdminAvatar admin={admin} />
          {!collapsed && (
