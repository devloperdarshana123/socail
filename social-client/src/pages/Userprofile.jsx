...
import api from "../services/api"; // ✅ FIX 1: axios hata ke api use karo
...
export default function UserProfile() {
  ...
  const fetchProfile = async () => {
    setLoading(true);
    try {
      // ✅ FIX 2: axios + manual token hata ke api.get use karo
      const [profileRes, postsRes] = await Promise.all([
  ...
  const handleFollow = async () => {
    // ✅ FIX 3: isPending argument hata do — slice sirf userId chahta hai
    const isPending = pendingRequests.includes(userId);
    const res = await dispatch(toggleFollowRequest({ userId }));
    if (toggleFollowRequest.fulfilled.match(res)) {
      toast.success(isPending ? "Request canceled!" : "Follow request sent!");
      fetchProfile();
    } else toast.error(res.payload || "Request failed!");
