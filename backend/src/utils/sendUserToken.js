...
export const sendUserToken = async (user, statusCode, res, options = {}, next) => {
  ...
  try {
    ...
    const refreshToken = await UserHelper.generateRefreshToken(user, deviceInfo, ipAddress, rememberMe);

    const refreshTTL = rememberMe ? REFRESH_TTL_REMEMBER_MS : REFRESH_TTL_MS;

    const safeUser = UserHelper.toSafeObject(user);

    logger.info("User tokens generated", { userId: user.id });
    ...
    logger.error("sendUserToken failed", { userId: user?.id, cause: err.message });
