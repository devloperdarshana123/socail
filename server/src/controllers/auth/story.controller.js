...
export const getStoriesFeed = asyncHandler(async (req, res) => {
  ...
  // Sabki active stories fetch karo (public)
...
export const deleteStory = asyncHandler(async (req, res, next) => {
  ...
  // Sirf media story ka Cloudinary se delete karo
...
export const deleteHighlight = asyncHandler(async (req, res, next) => {
  ...
  // Cloudinary se sab permanent copies delete karo
  const deletePromises = highlight.snapshots
    .filter((s) => s.publicId)
    .map((s) =>
      deleteFromCloudinary(s.publicId, s.resourceType).catch(() => {})
    );
...
export const toggleStoryLike = asyncHandler(async (req, res, next) => {
  ...
  if (viewerIdx > -1) {
    ...
    if (wasLiked) {
      ...
      // Like — pehle reaction tha ya nahi check karo
