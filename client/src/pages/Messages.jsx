...
//       flex: 1,
//     }}>
//       <style>{`@keyframes recPulse{0%,100%{opacity:0.3}50%{opacity:1}}`}</style>
...
//   { label: "Inappropriate content",  value: "nudity_or_sexual_content" },
//   { label: "Fake account",           value: "false_information" },
...
//       }}>
//         <p style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: "var(--color-text-primary)" }}>
//           Report user
//         </p>
...
//     border: `1.5px solid ${selected === r.value ? "#534AB7" : "var(--color-border-tertiary)"}`,
//     background: selected === r.value ? "#f0eeff" : "transparent",
//     fontSize: 13, cursor: "pointer",
//     color: selected === r.value ? "#534AB7" : "var(--color-text-primary)",
...
//           }}>Cancel</button>
//           <button disabled={!selected} onClick={() => onSubmit(selected)} style={{
//             flex: 1, padding: "10px 0", borderRadius: 10, border: "none",
//             background: selected ? "#D85A30" : "#ddd",
...
//   const [headerMenu,   setHeaderMenu]   = useState(false); // 3-dot menu
...
//   // ── Load conversations + following ───────────────────────────────────────
//   useEffect(() => { dispatch(fetchConversations()); }, [dispatch]);
//   // ── Open conversation from navigation state (Feed/Map se aaya) ───────────
...
//   useEffect(() => { if (myId) dispatch(fetchFollowing(myId)); }, [myId, dispatch]);
...
//     // Scroll bottom pe karo fresh load pe
//     setTimeout(() => {
...
//  // ── Check block status when conversation changes ─────────────────────────
// useEffect(() => {
//   if (!activeConvId || !otherId) {
//     setBlockStatus({ blocked: false, iBlockedThem: false });
//     return;
//   }
//   const fetchBlockStatus = async () => {
//     try {
//       const res = await fetch(`${BASE_URL}/api/v2/user/block-status/${otherId}`, {
//         credentials: "include",
//       });
...
//           iBlockedThem: data.data.iBlockedThem,
//         });
//       }
//     } catch (err) {
//       console.error("Block status fetch failed:", err);
//     }
//   };
//   fetchBlockStatus();
// }, [activeConvId, otherId]);

//   // ── Listen for block status response + block/unblock events ─────────────
//   useEffect(() => {
//     const s = getSocket();
//     if (!s) return;

//     const onBlockStatus = ({ targetUserId, blocked, iBlockedThem }) => {
//       if (targetUserId === otherId) setBlockStatus({ blocked, iBlockedThem });
//     };
//     const onBlockSuccess = ({ targetUserId }) => {
//       if (targetUserId === otherId) setBlockStatus({ blocked: true, iBlockedThem: true });
//     };
//     const onUnblockSuccess = ({ targetUserId }) => {
//       if (targetUserId === otherId) setBlockStatus({ blocked: false, iBlockedThem: false });
//     };
//     const onBlocked = ({ blockedBy }) => {
//       if (blockedBy === otherId) setBlockStatus({ blocked: true, iBlockedThem: false });
//     };
//     const onUnblocked = ({ unblockedBy }) => {
//       if (unblockedBy === otherId) setBlockStatus({ blocked: false, iBlockedThem: false });
//     };
...
//       s.off("user:block:success",   onBlockSuccess);
//       s.off("user:unblock:success", onUnblockSuccess);
...
//     if (editingMsg) {
//       if (!canEdit(editingMsg)) {
//         setEditError("Cannot edit — 15 minute window has passed.");
...
//     if (activeConvId) startTyping(activeConvId);
//   };
...
//     if (result?.success) {
//       setBlockStatus({ blocked: true, iBlockedThem: true });
//       showToast(`${otherParticipant?.fullName || "User"} blocked.`);
//     } else {
...
//   setShowReport(false);
//   try {
//     const res = await fetch(`${BASE_URL}/api/v2/user/report/${otherId}`, {
//       method: "POST",
...
//   const inputDisabled = blockStatus.blocked || imgUploading || audioUploading;
...
//             <p style={{ fontSize: 16, fontWeight: 600, marginBottom: 10, color: "var(--color-text-primary)" }}>
//               Messages
//             </p>
//             <label style={{
...
//                   return (
//                     <button key={uid} onClick={() => handleOpenWithUser(uid)}
//                       style={{
//                         display: "flex", alignItems: "center", gap: 12,
//                         padding: "12px 16px", width: "100%", textAlign: "left",
//                         border: "none", borderBottom: "0.5px solid var(--color-border-tertiary)",
...
//               )}
//               <Avatar
//                 name={otherParticipant?.fullName || otherParticipant?.username || "U"}
...
//                 </p>
//                 <p style={{ fontSize: 12, margin: 0, color: otherOnline ? "#1D9E75" : "var(--color-text-tertiary)" }}>
...
//                         onContextMenu={(e) => {
//                           e.preventDefault();
//                           if (!isOptimistic) setCtxMenu({ x: e.clientX, y: e.clientY, msg });
//                         }}
//                         onTouchStart={(e) => {
...
//                             : isMine ? "#534AB7" : "var(--color-background-secondary)",
//                           color: isDeleted
//                             ? "var(--color-text-tertiary)"
//                             : isMine ? "#fff" : "var(--color-text-primary)",
...
//                   {/* Voice recording bar replaces input */}
//                   {isRecording ? (
//                     <VoiceRecordBar
...
//                         😊
//                       </button>
//                       <button aria-label="Attach image" onClick={() => fileInputRef.current?.click()}
...
//       toggleVTT();
//     }}
//     disabled={audioUploading || isRecording} // isRecording = voice message wala
//     style={{
//       background: "none",
//       border: "none",
//       cursor: "pointer",
...
//                   {/* Send button — only show if text/image, not recording */}
//                   {!isRecording && (
