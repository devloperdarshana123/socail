...
// import { Search, X, User, Settings, LogOut, Menu, Bell, Heart, MessageCircle, UserPlus, UserCheck } from "lucide-react";
...
// import { selectTotalUnread, userCameOnline, userWentOffline, setOnlineUsers } from "../lib/redux/chatSlice";
...
// const selectAvatar        = (state) => state.userProfile?.avatar?.url || null;
...
//   // ── Notification state from Redux (managed by useSocketInit.js) ──────────
...
//   // ── Socket: ONLY online tracking + message toast ─────────────────────────
//   // NOTE: notification:new is handled ONLY in useSocketInit.js
//   // Navbar does NOT touch notification events — no duplicates.
//   useEffect(() => {
//     if (!user?._id) return;
...
//       // ── In-app toast for new messages (when NOT on /messages page) ───────
//       const handleMsgNotif = (data) => {
//         if (location.pathname === "/messages") return;
//         clearTimeout(msgToastTimer.current);
//         setMsgToast({
//           from:   data.sender?.fullName || data.sender?.username || "Someone",
//           preview: data.preview || "New message",
//           convId: data.conversationId,
//         });
//         msgToastTimer.current = setTimeout(() => setMsgToast(null), 4000);
//       };

//       // ── Online tracking → sync to chatSlice ──────────────────────────────
//       const handleOnlineList = (list) => dispatch(setOnlineUsers(list));
//       const handleOnline     = ({ userId }) => dispatch(userCameOnline({ userId }));
//       const handleOffline    = ({ userId }) => dispatch(userWentOffline({ userId }));

//       s.on("notification:message", handleMsgNotif);
//       s.on("online:list",          handleOnlineList);
//       s.on("user:online",          handleOnline);
//       s.on("user:offline",         handleOffline);

//       cleanup = () => {
//         s.off("notification:message", handleMsgNotif);
//         s.off("online:list",          handleOnlineList);
...
//   useEffect(() => {
//     const map = { "/feed": "Feed", "/explore": "Explore", "/messages": "Messages", "/saved": "Saved" };
...
//     if (type === "post_like"   || type === "like")           return <Heart size={15} fill="#ef4444" color="#ef4444" />;
//     if (type === "post_comment"|| type === "comment")        return <MessageCircle size={15} color="#4f46e5" />;
//     if (type === "follow_request")                           return <UserPlus size={15} color="#a855f7" />;
//     if (type === "follow_accepted")                          return <UserCheck size={15} color="#22c55e" />;
//     return <Bell size={15} color="#6b7280" />;
//   };

//   const notifBg = (type) => {
//     if (type === "post_like"    || type === "like")           return "#fff0f0";
//     if (type === "post_comment" || type === "comment")        return "#f0f4ff";
//     if (type === "follow_request")                            return "#fdf4ff";
//     if (type === "follow_accepted")                           return "#f0fdf4";
//     return "#f9fafb";
//   };

//   const notifText = (type) => {
//     if (type === "post_like"    || type === "like")           return "liked your post";
//     if (type === "post_comment" || type === "comment")        return "commented on your post";
...
//       {msgToast && (
//         <div
//           onClick={() => { navigate("/messages"); setMsgToast(null); }}
//           style={{
...
//             <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#fff2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
//               <MessageCircle size={16} color="#fff" />
//             </div>
//             <div style={{ minWidth: 0 }}>
//               <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: "#fff" }}>{msgToast.from}</p>
//               <p style={{ fontSize: 12, margin: 0, color: "rgba(255,255,255,0.75)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
//                 {msgToast.preview}
//               </p>
//             </div>
//             <button onClick={(e) => { e.stopPropagation(); setMsgToast(null); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.6)", marginLeft: 4, flexShrink: 0 }}>
//               <X size={14} />
//             </button>
//           </div>
//         </div>
...
//                 setDropdownOpen(false);
//                 if (!showNotifs) dispatch(markAllRead());
//               }}
...
//             {showNotifs && (
//               <div className="absolute right-0 w-80 rounded-2xl shadow-xl border border-gray-100 bg-white overflow-hidden"
...
//                     {notifications.map((n) => (
//                       <div key={n._id}
//                         className={`flex items-start gap-3 px-4 py-3 hover:bg-gray-50 border-b border-gray-50 last:border-0 ${!n.isRead ? "bg-blue-50/40" : ""}`}>
...
//                               {n.sender?.fullName || n.sender?.username || "Someone"}
//                             </span>{" "}
//                             {notifText(n.type)}
//                           </p>
//                           {n.text && (
...
//             <button onClick={() => { setDropdownOpen(!dropdownOpen); setShowNotifs(false); }}
//               className="p-1.5 rounded-xl hover:bg-gray-50">
//               <AvatarCircle size={30} fontSize={12} />
//             </button>

//             {dropdownOpen && (
//               <div className="absolute right-0 w-64 rounded-2xl shadow-xl border border-gray-100 overflow-hidden bg-white"
//                 style={{ top: "calc(100% + 8px)", zIndex: 9999 }}>
...
//                 </button>
//               ))}
//               <button onClick={() => { onCreatePost(); setMobileMenuOpen(false); }}
