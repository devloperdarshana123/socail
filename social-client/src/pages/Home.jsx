...
const Avatar = ({ src, name, size = "w-10 h-10", text = "text-sm" }) => {
  const imgSrc = src?.url || src;
  return imgSrc ? (
    <img src={imgSrc} alt={name} className={`${size} rounded-full object-cover shrink-0`} />
  ) : (
...
export default function Home({ showCreatePost, setShowCreatePost }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const saved = sessionStorage.getItem("homepage_scroll");
    if (!saved) return;
  ...
  const handleCreatePost = async (e) => {
    ...
    const res = await dispatch(createPost({ caption, image, video , location }));
      ...
      {showCreatePost && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex">
              <div className="w-[48%] bg-stone-50 border-r border-stone-100 flex flex-col items-center justify-center min-h-75 gap-4 p-6 relative">
                {imagePreview ? (
                  <>
                    <img src={imagePreview} alt="preview" className="w-full h-70 object-cover rounded-xl" />
                  ...
                  <>
                    <video src={videoPreview} controls className="w-full max-h-70 rounded-xl object-cover" />
