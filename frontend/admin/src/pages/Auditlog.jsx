...
import {
  ...
  CheckCircle, Eye, Activity, Clock,
...
// ─────────────────────────────────────────────────────────────────────────────
//  Constants — defined OUTSIDE component so they never re-create
// ─────────────────────────────────────────────────────────────────────────────
...
const ACTION_META = {
  ...
  "report.claimed":                 { icon: Eye,           label: "Report Claimed",           color: "text-sky-600",     bg: "bg-sky-50 border-sky-100" },
  "report.released":                { icon: LogOut,        label: "Report Released",          color: "text-slate-500",   bg: "bg-slate-50 border-slate-200" },
  "report.escalated":               { icon: AlertTriangle, label: "Report Escalated",         color: "text-red-600",     bg: "bg-red-50 border-red-100" },
  "report.under_review":            { icon: Clock,         label: "Report Under Review",      color: "text-amber-600",   bg: "bg-amber-50 border-amber-100" },
  "report.stale_claims_released":   { icon: RotateCcw,     label: "Stale Claims Released",    color: "text-slate-500",   bg: "bg-slate-50 border-slate-200" },
...
// ✅ OPTIMIZATION: Defined outside — never re-creates on render
...
const FALLBACK_ACTION_META = { icon: Activity, color: "text-slate-500", bg: "bg-slate-50 border-slate-200" };

function getActionMeta(action) {
  return ACTION_META[action] ?? { ...FALLBACK_ACTION_META, label: action };
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  Stat Card — memo so it skips re-render when props unchanged
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  AdminAvatar — memo + useMemo for URL
// ─────────────────────────────────────────────────────────────────────────────

const AdminAvatar = memo(function AdminAvatar({ name, avatarUrl }) {
  const [imgError, setImgError] = useState(false);

  // ✅ OPTIMIZATION: useMemo so URL string isn't rebuilt every render
  const uiAvatar = useMemo(
    () => `https://ui-avatars.com/api/?name=${encodeURIComponent(name ?? "Admin")}&background=1e3a5f&color=fff&size=64&bold=true&format=svg`,
    [name]
  );
...
// ─────────────────────────────────────────────────────────────────────────────
//  Log Row — memo so list doesn't re-render on pagination/filter state change
// ─────────────────────────────────────────────────────────────────────────────

const LogRow = memo(function LogRow({ log, onOpen }) {
  const meta    = getActionMeta(log.action);
  const catMeta = getCategoryMeta(log.category);
  const Icon    = meta.icon;

  // ✅ OPTIMIZATION: stable click handler per row
  const handleClick = useCallback(() => onOpen(log), [log, onOpen]);
    ...
    >
      ...
      <td className="px-4 py-3 sm:px-6 sm:py-4 hidden sm:table-cell">
        <div className="flex items-center gap-2">
          <AdminAvatar name={log.performedByName} avatarUrl={log.performedBy?.avatar?.url} />
          <div>
...
// ─────────────────────────────────────────────────────────────────────────────
//  Skeleton Row — memo (pure static, should never re-render)
// ─────────────────────────────────────────────────────────────────────────────
...
const EXCLUDED_TARGET_KEYS = ["commentText", "postCaption", "postType", "newStatus", "reason", "commentId", "postId"];

const DetailModal = memo(function DetailModal({ log, loading, onClose }) {
  ...
  // ✅ OPTIMIZATION: early return BEFORE computing meta (avoids wasted work)
            ...
            {log.targetMeta && Object.keys(log.targetMeta).length > 0 && (
              <div className="pt-2 border-t border-slate-100">
                ...
                <div className="bg-[#f8fafc] border border-slate-100 rounded-xl p-3 space-y-2">
                  ...
                  {/* ✅ OPTIMIZATION: EXCLUDED_TARGET_KEYS is a constant, not inline array */}
                  {Object.entries(log.targetMeta)
                    .filter(([k]) => !EXCLUDED_TARGET_KEYS.includes(k))
                    .map(([k, v]) =>
                      v != null && v !== "" ? (
...
export default function AuditLogPage() {
  ...
  // ✅ OPTIMIZATION: toast timer cleanup — prevents memory leak
  ...
  const handleOpenDetail = useCallback((log) => {
    dispatch(fetchAuditLogById(log.id));
    setModalOpen(true);
  }, [dispatch]);
  ...
  // ✅ OPTIMIZATION: useMemo — countMap avoids .find() on every render
  const countMap = useMemo(() => {
    const breakdown = stats?.categoryBreakdown ?? [];
    return Object.fromEntries(breakdown.map((c) => [c._id, c.count]));
  }, [stats]);

  // ✅ OPTIMIZATION: useMemo — STAT_CARDS won't re-create unless counts change
  const STAT_CARDS = useMemo(() => [
    ...
    },
    {
      ...
      value: countMap["auth"] ?? 0,
      accent: { cardBg: "bg-emerald-50 border-emerald-100 hover:bg-emerald-100/50 hover:border-emerald-200", icon: "text-emerald-500" },
    },
  ], [totalLogs, countMap]);

  // ✅ OPTIMIZATION: useMemo — actionOptions won't re-create unless constants change
  const actionOptions = useMemo(
    () => constants?.actions ? Object.values(constants.actions) : Object.keys(ACTION_META),
    [constants]
  );

  // ✅ OPTIMIZATION: useMemo — pagination buttons list
  ...
  // ✅ OPTIMIZATION: stable skeleton array — won't re-create on each render
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px]">
                ...
                <tbody>
                  {loading
                    ? skeletonRows.map((i) => <SkeletonRow key={i} />)
                    : logs.length === 0
                    ? (
                      <tr>
                        <td colSpan={6}>
                    ...
                    : logs.map((log) => (
                      <LogRow key={log.id} log={log} onOpen={handleOpenDetail} />
                    ))
                  }
                </tbody>
              </table>
          ...
          {!loading && totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4
              ...
              <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap justify-center order-1 sm:order-2">
                ...
                {/* ✅ OPTIMIZATION: useMemo replaces IIFE in JSX */}
                {pageNumbers.map((p) => (
                  <button key={p}
