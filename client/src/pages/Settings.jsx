...
function PasswordTab() {
  ...
  const handleUpdate = () => {
    if (!matchOk) return;
    dispatch(updatePassword({ oldPassword: oldPwd, newPassword: newPwd }))
      .unwrap()
     .then(() => {
  setOldPwd(""); setNewPwd(""); setConfPwd("");
  toast.success("Password updated successfully!");
})
.catch((err) => toast.error(err || "Password update failed"));
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <PwdField label="New Password" placeholder="Min 8 characters"
          value={newPwd} onChange={(e) => setNewPwd(e.target.value)}
          show={showNew} onToggle={() => setShowNew(!showNew)} />
        <PwdField label="Confirm New Password" placeholder="Repeat new password"
