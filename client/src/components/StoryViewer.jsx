...
const durationRef = useRef(STORY_DURATION);
  ...
  const isOwn  = story?.author?._id === currentUser?._id ||
...
const handleReact = (emoji) => {
  ...
  setShowReact(false);
  setPaused(false);
  dispatch(reactToStoryThunk({ storyId: story._id, reaction: emoji }));
};
...
const handleLike = () => {
  ...
  dispatch(optimisticLike({ storyId: story._id, liked: newLiked }));
  dispatch(toggleStoryLike(story._id)).then((res) => {
    ...
    if (fetchMyHighlights.fulfilled.match(res)) {
      const saved = new Set(
        res.payload
          .filter((h) =>
            h.snapshots?.some(
              (s) => s.storyId?.toString() === story._id?.toString()
            )
          )
          .map((h) => h._id)
      );
...
const createNewHighlight = async () => {
  ...
  if (createHighlight.fulfilled.match(result)) {
    setSavedHighlights((prev) => new Set([...prev, result.payload._id]));
    setNewTitle("");
                  ...
                  {highlightLoading ? (
                    ...
                    <div className="space-y-1">
                     {reduxHighlights.map((h) => {
                        const isSaved = savedHighlights.has(h._id);
                      const cover = h.coverImage || h.snapshots?.find(s => s.url)?.url || null;
                        return (
                          <button key={h._id}
                            onClick={() => toggleHighlight(h._id)}
            ...
            {showViewers && (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <p className="font-bold text-[#2d1f0f]">Viewers ({(viewersMap[story?._id]?.viewers || []).length})</p>
                  ...
                  {(viewersMap[story?._id]?.viewers || []).map((v, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full overflow-hidden bg-[#e8d5be] shrink-0">
                        {v.user?.avatar?.url
                          ? <img src={v.user.avatar.url} alt="" className="w-full h-full object-cover" />
                          : <div className="w-full h-full flex items-center justify-center bg-[#c09a6e]">
                              <span className="text-white text-xs font-bold">{v.user?.fullName?.[0]}</span>
                            </div>
                        }
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-[#2d1f0f]">{v.user?.fullName}</p>
                        <p className="text-xs text-[#8b7355]">@{v.user?.username}</p>
                      </div>
