...
export const REFRESH_TOKEN_TTL_REMEMBER_MS = 30 * 24 * 60 * 60 * 1000;
...
export const sendToken = async (user, statusCode, res, options, next) => {
  ...
  try {
    ...
    const refreshToken = await generateRefreshToken(user, deviceInfo, ipAddress, rememberMe);

    logger.info("Token generation timing", {
      userId: user.id, elapsedMs: Date.now() - t0,
    });
    ...
    logger.info("Tokens generated successfully", {
      userId: user.id, statusCode, rememberMe, elapsedMs: Date.now() - t0,
    });
