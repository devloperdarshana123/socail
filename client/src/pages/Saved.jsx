...
function MasonryGrid({ posts, onPostClick, loading }) {
  // Mobile pe 2 cols, desktop pe 3 cols — posts usi hisab se distribute honge
  const [numCols, setNumCols] = useState(
    typeof window !== "undefined" ? (window.innerWidth < 768 ? 2 : 3) : 2
  );
      ...
      {loading && posts.length === 0
        ...
        : cols.map((col, ci) => (
            <div key={ci} className="flex flex-col gap-2 md:gap-3">
              {col.map((post) => (
                <SavedCard key={post.id} post={post} onClick={onPostClick} />
              ))}
            </div>
          ))
      }
