...
export default function EditDraftModal({ post, onClose, onSaved }) {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await dispatch(
        updateDraft({ postId: post.id, caption, media: mediaItems })
      ).unwrap();
      toast.success("Draft updated! 📝");
      onSaved({ type: "draft", postId: post.id, caption });
    } catch {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Phir publish
      await dispatch(publishDraftPost(post.id)).unwrap();
      toast.success("Post published! 🎉");
      onSaved({ type: "published", postId: post.id });
    } catch {
