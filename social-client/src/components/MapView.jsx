...
export default function MapView({ searchQuery, selectedCategory, onSellerClick, onSearchChange, onCategoryChange }) {
  ...
  // ✅ userCoords ka ref — fetchSellers mein stale closure se bachne ke liye
  const userCoordsRef = useRef(null);

  // ✅ api service use kar raha hai — no manual token
  const fetchSellers = useCallback(async (lat, lng) => {
    setLoading(true);
    setError(null);
    try {
      const params = { category: selectedCategory || "all" };
      if (lat && lng) { params.lat = lat; params.lng = lng; }
      const res = await api.get("/marketplace/sellers", { params });
      setSellers(res.data.sellers || []);
    } catch (err) {
  ...
  // ✅ Geolocation with cleanup — memory leak nahi hoga
  useEffect(() => {
    let cancelled = false;
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          // ✅ Location update — api service use kar raha hai
          api.put("/marketplace/location", {
            lat: pos.coords.latitude,
            lng: pos.coords.longitude,
          }).catch(() => {});
        },
        () => {
          if (!cancelled) fetchSellers();
        }
      );
    } else {
      fetchSellers();
  ...
  // ✅ Follow/Unfollow — local state update, no refetch
  const handleFollow = async (sellerId, isPending, isFollowing, e) => {
    e.stopPropagation();
    try {
      if (isFollowing) {
        await api.delete(`/follow/${sellerId}/unfollow`);
        setSellers((prev) =>
          prev.map((s) =>
            s._id === sellerId
              ? { ...s, isFollowing: false, isPending: false, followersCount: (s.followersCount || 1) - 1 }
              : s
          )
        );
      } else if (isPending) {
        await api.delete(`/follow/${sellerId}/cancel`);
        setSellers((prev) =>
          prev.map((s) =>
            s._id === sellerId ? { ...s, isPending: false } : s
          )
        );
      } else {
        await api.post(`/follow/${sellerId}/send`);
        setSellers((prev) =>
          prev.map((s) =>
            s._id === sellerId ? { ...s, isPending: true } : s
          )
        );
      }
    } catch (err) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {error && (
        <div style={{
          position: "absolute", top: 12, left: "50%", transform: "translateX(-50%)",
          zIndex: 1000, background: "#fff1f2", padding: "6px 16px",
          borderRadius: 999, fontSize: 12, color: "#be123c",
          boxShadow: "0 2px 8px rgba(0,0,0,0.12)", border: "1px solid #fecdd3",
          display: "flex", alignItems: "center", gap: 8,
        }}>
          {error}
          <button
            onClick={() => setError(null)}
            style={{ background: "none", border: "none", cursor: "pointer", color: "#be123c", fontWeight: 700 }}
          >
            ✕
          </button>
        </div>
