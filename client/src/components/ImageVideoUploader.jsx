...
const ImageVideoUploader = forwardRef(function ImageVideoUploader(
  { onUploadComplete, isDark = false },
  ref
) {
  ...
  // ✅ Single delete call — backend ko bhejo turant
  const deleteFromServer = async (item) => {
    try {
      await fetch(`${API_BASE}/api/v2/posts/media/unused`, {
        method: "DELETE",
      ...
      console.warn("Media delete failed (cron will clean it later):", err);
    }
  };
  ...
  const removeItem = (idx) => {
    ...
    if (removed) deleteFromServer(removed); // ✅ turant cloudinary se delete
  };

  // ✅ Parent (PostCreatorModal) ko expose kiye gaye methods
  useImperativeHandle(ref, () => ({
    cleanupAll: async () => {
      if (submittedRef.current) return; // post submit ho gaya, kuch delete mat karo
      if (items.length === 0) return;
      try {
        await fetch(`${API_BASE}/api/v2/posts/media/bulk-delete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            items: items.map((m) => ({ publicId: m.publicId, resourceType: m.resourceType })),
          }),
        });
      } catch (err) {
        console.warn("Bulk cleanup failed (cron will catch it):", err);
      }
    },
    markSubmitted:   () => { submittedRef.current = true; },
    unmarkSubmitted: () => { submittedRef.current = false; },
    resetLocal:      () => { setItems([]); setProgress({}); setErrors([]); submittedRef.current = false; },
    getItems:        () => items,
  }));
