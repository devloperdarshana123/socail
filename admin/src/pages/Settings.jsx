...
// FIX: Wrapped with memo(). active is a string, onChange is a stable useCallback.
...
export default function SettingsPage() {
  ...
  // FIX: sessState.data.filter((s) => !s.isCurrent) was inlined in JSX,
  // running a full .filter() on every render. One useMemo instead.
  const hasOtherSessions = useMemo(
    () => sessState.data.some((s) => !s.isCurrent),
    [sessState.data],
  );

  // ── Handlers (all stabilized with useCallback) ──────────────────────────────

  // FIX: All handlers were plain functions recreated on every render.
  // Wrapped with useCallback so child components receive stable references.
