...
import CommentSection from "../components/CommentSectionV2"; // ✅ Added
import {
  fetchTrendingPosts, fetchSuggestedUsers,
  searchAll, clearSearch,
  toggleFollowRequest, likeTrendingPost, commentTrendingPost, deleteTrendingPost,
...
export default function Explore() {
 ...
 const handleFollow = async (userId) => {
  const isFollowing = pendingRequests.includes(userId);
  const result = await dispatch(toggleFollowRequest({ userId }));
  ...
  const confirmDelete = async () => {
    ...
    const result = await dispatch(deleteTrendingPost(postToDelete));
    if (deleteTrendingPost.fulfilled.match(result)) toast.success("Post deleted!");
                ...
                {trendingPosts.map((post) => (
                  <div key={post._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-4 py-3">
                      ...
                      >
                        <Avatar src={post.author?.avatar?.url || post.author?.avatar} name={post.author?.name} />
                    ...
                    {/* ✅ FIXED: CommentSectionV2 properly integrated */}
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {u.avatar ? (
                      ...
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
                        style={{ background: "linear-gradient(135deg, #c8956c, #a07050)", color: "#ffffff" }}>
