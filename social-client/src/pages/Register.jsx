...
const res = await fetch(`${import.meta.env.VITE_SERVER}/auth/google`, {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name || !form.email || !form.password) { toast.error("Please enter all fields"); return; }
  ...
  // ✅ Google Register Handler — signInWithGooglePopup use karo (fixed)
  const handleGoogleRegister = async () => {
    setGoogleLoading(true);
    try {
      const result = await signInWithGooglePopup();

      // Null = user ne popup band kiya ya redirect chal raha hai
      if (!result) {
        setGoogleLoading(false);
        return;
      }
